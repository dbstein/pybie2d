...
from pybie2d.kernels.high_level.laplace import Laplace_Kernel_Self_Apply, Laplace_Kernel_Self_Form
...
gy_numexpr = cMATy.dot(charge) + dMATy.dot(dipstr)
time_apply = %timeit -o cMAT.dot(charge)+dMAT.dot(dipstr); cMATx.dot(charge)+MATx.dot(dipstr); cMATy.dot(charge)+MATy.dot(dipstr)
...
# using numba
print('Testing Numba (Apply)')
pot_numba = Laplace_Kernel_Self_Apply(source, charge=charge, backend='numba')
time_numba =  %timeit -o Laplace_Kernel_Self_Apply(source, charge=charge, backend='numba')
