...
class Modified_Helmholtz_SLP_Self(object):
    """
    ...
    def Form(self, k):
        if k not in self.mats.keys():
            x = self.boundary.x
            y = self.boundary.y
            nx = self.boundary.normal_x
            ny = self.boundary.normal_y
            w = self.boundary.weights
            self.mats[k] = Modified_Helmholtz_SLP_Self_Form(x, y, nx, ny, w, k, self.alpert_I)
        return self.mats[k]
...
def Modified_Helmholtz_SLP_Self_Form(x, y, nx, ny, w, k, aI):
    N = x.shape[0]
    sel1 = np.arange(N)
    sel2 = sel1[:,None]
